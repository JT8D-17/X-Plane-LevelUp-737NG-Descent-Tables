...
function B738_variant_test_active_variant()
	if B738DR_b737_variant ~= nil then
		local variant_id = math.floor(B738DR_b737_variant + 0.5)
...
function B738_variant_test_direct_segment(model, model_id, low_alt, high_alt)
	...
	if table_ref == nil or high_alt == nil or low_alt == nil or high_alt > model.direct_alt[#model.direct_alt] then
		return nil
	end

	local weight = simDR_total_weight / 2.20462
...
function B738_variant_test_take_alt_dist(x_idx_alt, x_spd_alt, x_spd_wnd_alt, x_flap)
	...
	if variant_id == nil or x_flap ~= 0 then
		return nil
	end
